...
type Contributor = {
  ...
  name: string;
  percent: number;
  note?: string; // human-attested contribution git history can't see
};
...
// projects.json is generated by generate-projects.ts from real GitHub
// contributor stats — re-run that script to refresh these numbers.
  ...
  return (
    <main className="relative min-h-screen text-foreground">
      ...
      <header className="fixed left-1/2 top-5 z-50 -translate-x-1/2">
        <div
          className="glass flex w-fit items-center gap-2 whitespace-nowrap rounded-full px-3 py-2 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.6)] md:gap-4 md:px-5"
          style={{ borderImage: "linear-gradient(135deg, rgba(197,160,89,0.7), rgba(253,245,230,0.4)) 1" }}
        >
          <a href="#top" className="flex shrink-0 items-center gap-2 pr-2">
          ...
          <nav className="flex shrink-0 flex-nowrap items-center gap-1 text-[10px] uppercase tracking-[0.22em] text-parchment/80 sm:gap-2 md:gap-3 md:text-[11px]">
            {[
              ["About", "#about"],
              ["Skills", "#skills"],
              ["Work", "#projects"],
              ...
              <a
                key={href}
                href={href}
                className="shrink-0 rounded-full px-3 py-1.5 transition-all duration-300 hover:bg-primary/20 hover:text-primary md:px-4"
              >
                {label}
              </a>
            ))}
          ...
          <a
            href="/Mahnoor_Naeem_CV.pdf"
            download="Mahnoor_Naeem_CV.pdf"
            className="ml-1 flex shrink-0 items-center gap-1.5 rounded-full border border-primary/50 px-3 py-1.5 text-[10px] uppercase tracking-[0.22em] text-primary transition-all duration-300 hover:bg-primary/15 md:px-4 md:text-[11px]"
          >
            CV
                    ...
                    <div className="mt-1 space-y-1">
                      {p.contributors.map((contributor) => (
                        <div key={contributor.login}>
                          ...
                          />
                          {contributor.note && (
                            <p className="mt-1 text-[11px] italic text-parchment/60">
                              ✦ {contributor.note} <span className="not-italic text-parchment/40">(not reflected in % — based on lines changed only)</span>
                            </p>
                          )}
                        </div>
                      ))}
